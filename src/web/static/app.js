import { TrackVisualizer } from "./visualizer.js";
...
const elements = {
  ...
  upload: document.querySelector("#videoUpload"),
...
function bindTheme() {
  ...
  const savedTheme = queryTheme || localStorage.getItem(THEME_KEY) || "microscope-dark";
...
function bindControls() {
  ...
  elements.upload.addEventListener("change", () => {
    elements.uploadLabel.textContent = elements.upload.files?.[0]?.name || "上传新视频";
  });
...
async function loadDefaults() {
  try {
    ...
    populateSelect(elements.videoSelect, state.defaults.videos, "未发现视频");
...
function normalizeFormData(formData) {
  for (const name of ["use_cuda", "half_precision", "save_video", "evaluation_enabled", "draw_labels"]) {
    formData.set(name, elements.form.elements[name].checked ? "true" : "false");
  }
  if (!elements.upload.files.length) {
    formData.delete("video_file");
  }
  if (!formData.get("max_frames")) {
    formData.delete("max_frames");
  }
}
...
async function loadLogs() {
  if (!state.currentJobId) return;
  const response = await fetch(`/api/jobs/${state.currentJobId}/logs?tail=180`);
  if (response.ok) setLog(await response.text());
}

async function loadResults() {
  if (!state.currentJobId || state.resultLoadedFor === state.currentJobId) return;
  const response = await fetch(`/api/jobs/${state.currentJobId}/results`);
  const payload = await readJsonResponse(response);
...
async function loadTracks() {
  ...
  const response = await fetch(`/api/jobs/${state.currentJobId}/tracks?stride=${encodeURIComponent(stride)}`);
  const payload = await readJsonResponse(response);
...
function renderArtifacts(artifacts = {}) {
  ...
  const nodes = order.map((kind) => {
    const item = artifacts[kind] || { exists: false, url: "#", size: 0 };
    const row = document.createElement("div");
    row.className = "artifact-row";
    const link = document.createElement("a");
    link.href = item.url;
    link.textContent = artifactLabel(kind);
    link.setAttribute("aria-disabled", String(!item.exists));
    if (!item.exists) link.removeAttribute("href");
