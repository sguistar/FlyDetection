...
Fly MOT 是一个面向果蝇视频的多目标检测、身份关联、轨迹修复、事件诊断和评估项目。当前推荐工作流以 `src/` 下的模块化管线为准，并提供本地 Web GUI 作为可视化控制台。 
...
- 指标评估，支持基础 tracking 指标、点匹配评估、HOTA-like 分项和 hard-case 分析。
- ReID 数据准备与外观编码器训练。
...
```text
.
├─ main.py                    # src/main.py 的兼容入口，便于直接运行默认管线
├─ src/
...
- Python `>=3.9`
- `uv`
- Windows / Linux 均可运行核心 Python 管线；CUDA 可选
- NVIDIA GPU 可显著加速 YOLO/Torch 推理与训练
